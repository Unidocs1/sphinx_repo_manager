...
Now that the candidates and teams have been selected its time to create the match. In order to eliminate race conditions resulting from the assignment of the same ticket in different matches a transactional database capable of performing atomic operations is required. Xsolla Backend Core's ticket processor uses the Redis database for this purpose.
